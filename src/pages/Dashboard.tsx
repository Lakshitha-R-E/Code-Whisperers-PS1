...
const Dashboard: React.FC = () => {
  ...
  const districts = useMemo(() => {
    const dSet = new Set(rawWatersheds.map(w => w.properties.district));
    return Array.from(dSet).sort();
  }, [rawWatersheds]);

  const availableWatersheds = useMemo(() => {
    if (!district) return rawWatersheds;
    return rawWatersheds.filter(w => w.properties.district === district);
  }, [rawWatersheds, district]);

  const filteredProjects = useMemo(() => {
    return rawProjects.filter(p => {
      if (officer && !officer.isAdmin && p.district !== officer.district) return false;
      if (district && p.district !== district) return false;
      if (watershedId && p.watershed_id !== watershedId) return false;
      return true;
    });
  ...
  const filteredWatersheds = useMemo(() => {
    return availableWatersheds.filter(w => {
      if (officer && !officer.isAdmin && w.properties.district !== officer.district) return false;
      if (watershedId && w.properties.id !== watershedId) return false;
      return true;
    });
  }, [availableWatersheds, officer, watershedId]);

  const scopedIds = useMemo(() => new Set(filteredProjects.map(p => p.id)), [filteredProjects]);
  const filteredAlerts = useMemo(() => alerts.filter(a => scopedIds.has(a.project_id)), [alerts, scopedIds]);
  const filteredEvidence = useMemo(() => evidence.filter(e => scopedIds.has(e.project_id)), [evidence, scopedIds]);

  const stats = useMemo(() => {
    const completed = filteredProjects.filter(p => p.status === 'Completed').length;
    const pct = filteredProjects.length ? Math.round((completed / filteredProjects.length) * 100) : 0;
    const avgTrust = computeAverageTrustScore(filteredEvidence);
    const spatiallyVerified = filteredEvidence.filter(e => e.gps_check === 'pass').length;
    const spvPct = filteredEvidence.length ? Math.round((spatiallyVerified / filteredEvidence.length) * 100) : 0;
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="border-b border-gray-200 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5 bg-white border border-gray-200 rounded-lg p-2 shadow-sm">
          {officer?.isAdmin && (
            <select
              value={district}
              onChange={e => { setDistrict(e.target.value); setWatershedId(''); }}
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-3.5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-text-dark">Satellite Engine:</span>
        ...
        <div className="flex items-center gap-2">
          <span className="font-semibold text-text-dark">SRISHTI-DRISHTI:</span>
      ...
      <div className="bg-primary-700 text-white rounded-lg px-5 py-3 flex flex-wrap gap-4 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold">{filteredProjects.filter(p => p.status === 'Delayed').length}</span>
        </div>
        <div className="text-primary-400 hidden sm:block">|</div>
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold">
            {[...new Set(filteredWatersheds.map(w => w.properties.state))].join(', ')}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-accent-600 font-semibold">{filteredAlerts.length} active</span>
          </div>
          <AlertsList alerts={filteredAlerts} limit={5} />
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredWatersheds.map(ws => {
                const wsProjects = filteredProjects.filter(p => p.watershed_id === ws.properties.id);
